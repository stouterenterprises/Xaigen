...
<div class="container"><h1><?=$isLoggedIn ? 'My Gallery' : 'Public Gallery'?></h1><div class="gallery-list"><?php foreach($items as $g): $isSucceeded=strtolower((string)$g['status'])==='succeeded'; $hasOutput=!empty($g['output_path']); $mediaViewHref='/app/media.php?id='.urlencode((string)$g['id']); $statusClass='status-'.preg_replace('/[^a-z0-9_-]/','',strtolower((string)($g['status'] ?? 'unknown'))); ?>
