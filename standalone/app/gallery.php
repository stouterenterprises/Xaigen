...
try {
  ...
  $isLoggedIn = !empty($_SESSION['admin_user_id']) || $currentUser;
...
?>
<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Gallery</title><link rel="stylesheet" href="/app/assets/css/style.css?v=<?=urlencode((string)$styleVersion)?>"></head><body>
<nav class="site-nav"><div class="container nav-inner"><a class="brand" href="/">Xaigen</a><button class="menu-toggle" aria-expanded="false" aria-controls="nav-links">Menu</button><div id="nav-links" class="nav-links"><a href="/">Home</a><a href="/app/create.php">Generator</a><a href="/app/gallery.php">Gallery</a><a href="/app/customize.php">Customize</a><?php if($currentUser): ?><a href="/app/logout.php">Logout (<?=htmlspecialchars((string)$currentUser['username'])?>)</a><?php elseif(!empty($_SESSION['admin_user_id'])): ?><a href="/admin/index.php">Admin</a><a href="/admin/logout.php">Logout (Admin)</a><?php else: ?><a href="/app/login.php">Login</a><?php endif; ?></div></div></nav>
<div class="container"><h1><?=$isLoggedIn ? 'My Gallery' : 'Public Gallery'?></h1><?php if($pageError): ?><div class="banner">Unable to load gallery: <?=htmlspecialchars($pageError)?></div><?php endif; ?><div class="gallery-list"><?php foreach($items as $g): $isSucceeded=strtolower((string)$g['status'])==='succeeded'; $hasOutput=!empty($g['output_path']); $mediaViewHref='/app/media.php?id='.urlencode((string)$g['id']); $statusClass='status-'.preg_replace('/[^a-z0-9_-]/','',strtolower((string)($g['status'] ?? 'unknown'))); ?>
