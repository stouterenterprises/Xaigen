...
- `lib/xai.php` now normalizes OpenRouter base URLs so values like `https://openrouter.ai` are automatically expanded to `https://openrouter.ai/api/v1`, preventing 404 HTML responses when `/images/generations` or `/videos/generations` calls are made.
- `lib/xai.php` request error messages are now labeled by the active provider (for example, `OPENROUTER ...`) instead of always saying `xAI`, making provider-routing diagnostics clearer in gallery/tick failure messages.
- `lib/xai.php` error parsing now reads string-based provider error payloads (`error`, `code`) in addition to nested `error.message`, so xAI model-access failures like `The model ... does not exist or your team ... does not have access` surface directly instead of opaque JSON blobs.
- `lib/xai.php` now appends an explicit hint when OpenRouter returns `404` for `POST /videos/generations`, clarifying that the selected model/endpoint combination may not support OpenRouter video generation and suggesting xAI video models as a fallback.
- `normalize_provider_base_url()` in `lib/xai.php` now force-normalizes OpenRouter host variants (`openrouter.ai`, `www.openrouter.ai`, and other `*.openrouter.ai` values) to `https://openrouter.ai/api/v1`, including legacy `/v1` entries, preventing accidental website HTML `404` responses when API key settings are saved with non-API paths.
...
- `lib/xai.php` video generation now retries xAI model-access 404 errors across multiple fallback candidates (accessible `/models` video ids first, then known Grok aliases) instead of a single alias attempt, preventing hard failures when both `grok-2-video` and `grok-video` are unavailable but another accessible video model exists.
...
- OpenRouter text/chat-style models (Dolphin/Venice/Hermes/Qwen/Llama/Mixtral families) are now re-activated for generator dropdown visibility; generation requests targeting those models are routed through `/chat/completions` using the generator prompt + negative prompt, then marked failed with the returned chat guidance because chat endpoints do not emit media files.
- `api/tick.php` now accepts additional async id shapes (including numeric ids, `generation_id`, nested `data/result/response` variants) before failing a running job, reducing false `Running job is missing external_job_id` failures when providers return non-string generation identifiers.
- `lib/xai.php` now retries OpenRouter requests with model-id fallbacks when providers return `HTTP 400 ... not a valid model ID` (including legacy `josiefied-qwen3-8b` → `josiefied/qwen3-8b`), and migration `0012_fix_openrouter_legacy_model_ids.sql` auto-corrects existing saved model keys.
- `openrouter_model_key_candidates()` in `lib/xai.php` now includes expanded known-mappings for all seeded OpenRouter models so the correct provider-qualified model IDs are tried first: `nous-hermes-2-mixtral-8x7b` → `nousresearch/nous-hermes-2-mixtral-8x7b-dpo`, `dolphin-2.5-mixtral` → `cognitivecomputations/dolphin-mixtral-8x7b`, `josiefied/qwen3-8b` → `qwen/qwen3-8b`. Both the original shorthand key and the first-hyphen-slash variant are mapped so existing database rows and legacy references resolve correctly.
- Migration `0013_fix_openrouter_model_ids.sql` updates stored `model_key` values in the `models` table to their correct OpenRouter IDs (`nousresearch/nous-hermes-2-mixtral-8x7b-dpo`, `cognitivecomputations/dolphin-mixtral-8x7b`, `qwen/qwen3-8b`) so fallback resolution is no longer needed for these models. The Josiefied Qwen3-8B model is not available on OpenRouter; it is remapped to the official `qwen/qwen3-8b`.
- `api/tick.php` no longer applies the 60-second repeated-404 fail-fast rule to xAI polling errors; xAI jobs now continue running until the standard generation timeout, avoiding premature failures when provider generation ids return transient 404s during warm-up.
