...
- `normalize_provider_base_url()` in `lib/xai.php` now force-normalizes `openrouter.ai` base URLs to `https://openrouter.ai/api/v1` unless they already include `/api/v1`, preventing accidental website HTML `404` responses when API key settings are saved as `https://openrouter.ai` or `https://openrouter.ai/api`.
